...
\begin{algorithm}
	...
	\begin{algorithmic}
		...
		\Ensure $AI_S(f)$
		\State
		\State \textbf{Function} \textit{construct\_S\_f\_and\_g\_generators}$(x,n,S)$
		\State \quad function body
		\State \quad \Return $matrix_1, matrix_2, matrix_3$ 
		\State \textbf{End Function}
		\State
		
		\State $m_{max}, D^{\leq n_{max,res}}\gets \mbox{\textit{read\_values\_from\_file}}()$
		\State $G_{rows,n}^{f_{res},S}, G_{rows,n}^{(f\oplus 1)_{res},S}, G_{n,n}^S  \leftarrow construct\_S\_f\_and\_g\_generators\left(x,n,S\right)$
		\State $immunity_f, immunity_{f+1} \leftarrow ($\\
		$\ find\_deg\_smallest\_S\_annihilator(G_{rows,n}^{f_{res},S}, G_{n,n}^S,D^{\leq n_{max, res}})$\\
		$\ |\ find\_deg\_smallest\_S\_annihilator(G_{rows,n}^{(f\oplus 1)_{res},S}, G_{n,n}^S, D^{\leq n_{max, res}})$\\
		$)$
