...
\begin{algorithm}\label{alg:RMApproach}
	...
	\begin{algorithmic}[1]
		...
		\State $ef_f\gets \mbox{\textsc{row\_echelon\_form\_last\_}}D_i^n\mbox{\textsc{\_rows}}\left(\mbox{\textsc{merge}}\left(ef_f, G_{n,n}^{f_Z^{(o)},S}[previousIndex:D_i^n]\right)\right)$
		\State $ef_S \gets \mbox{\textsc{row\_echelon\_form\_last\_}}D_i^n\mbox{\textsc{\_rows}}\left(\mbox{\textsc{merge}}\left(ef_S, G_{n,n}^S[previousIndex:D_i^n]\right)\right)$
