...
\begin{theorem}\label{upperBoundRestricted}
...
\begin{proof}
	...
	\end{itemize}
	Taking the bigger rank between the ones found in the conditions above, we see that the upper bound for the algebraic immunity on the restricted set $S$ is reached at the degree $r_{upper}$ such that $rank\left(G_{r_{upper}, n}^S\right) > |S| / 2$.
\end{proof}
