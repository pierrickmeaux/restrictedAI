...
\begin{remark}\label{rem:gfMatrix}
	Let $g \in \Bn{n}$ and $\hat{a} = (a_I)_{I \subseteq {1,2,...,n}}$ be the vector of the coefficients of the $ANF$ of $g$, as a consequence of the construction of $G_{r,n}^{f,S}$, we have that
	\begin{equation}
	\begin{aligned}
...
In this section, we present an algorithm to determine the algebraic immunity restricted to a set $S \subseteq \mathbb{F}_2^n$ with a better time and space complexity of the one presented in Section \ref{sec:RMapproach}.\\ 
...
\begin{definition}[Vandermonde matrix]\label{def:tVandermonde}
...
\lbnote{I will take care later of transposing the Vanderonde matrix}\\
