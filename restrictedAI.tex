...
With these definitions in mind, we re-state the algorithm procedure as follows. Let $Z = supp\left(f^{(o)}\right) \cap S$ and $1\leq i \leq D_n^n$, the algorithm works by constructing the matrix $V_{E_i, Z_i}$, finding non non-zero annihilators restricted on $S$ of the Boolean function with support $Z_i$, by taking elements of the kernel of $V_{E_i, Z_i}$. If such non-zero annihilators exists, it checks if any of therm are also non-zero-annihilator restricted on $f$ of the full function $f^{(o)}$ with support $Z$.  
